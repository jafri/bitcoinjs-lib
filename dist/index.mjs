...
var typeforce = typef;
var ZERO32 = NBuffer.alloc(32, 0);
var EC_P = NBuffer.from(
...
import * as bs58check2 from "bs58check";
var OPS6 = OPS;
...
function p2wsh(a, opts) {
  ...
  const _address = value(() => {
    const result = bech322.decode(a.address);
    const version = result.words.shift();
    const data = bech322.fromWords(result.words);
    return {
      version,
      prefix: result.prefix,
      data: Buffer.from(data)
  ...
  prop(o, "address", () => {
    if (!o.hash)
      return;
    const words = bech322.toWords(o.hash);
    words.unshift(0);
    return bech322.encode(network.bech32, words);
  });
  prop(o, "hash", () => {
    if (a.output)
...
import { Buffer as NBuffer3 } from "buffer";
...
import { Buffer as NBuffer2 } from "buffer";
...
function tapleafHash(leaf) {
  ...
  return taggedHash(
    "TapLeaf",
    NBuffer2.concat([NBuffer2.from([version]), serializeScript(leaf.output)])
  );
}
function tapTweakHash(pubKey, h2) {
  return taggedHash(
    "TapTweak",
    NBuffer2.concat(h2 ? [pubKey, h2] : [pubKey])
  );
}
function tweakKey(pubKey, h2) {
  if (!NBuffer2.isBuffer(pubKey))
    return null;
  if (pubKey.length !== 32)
    return null;
  if (h2 && h2.length !== 32)
  ...
  return {
    parity: res.parity,
    x: NBuffer2.from(res.xOnlyPubkey)
  };
}
function tapBranchHash(a, b) {
  return taggedHash("TapBranch", NBuffer2.concat([a, b]));
}
function serializeScript(s) {
  const varintLen = varuint.encodingLength(s.length);
  const buffer = NBuffer2.allocUnsafe(varintLen);
  varuint.encode(s.length, buffer);
  return NBuffer2.concat([buffer, s]);
}
...
function p2tr(a, opts) {
  ...
  prop(o, "witness", () => {
    ...
    if (hashTree && a.redeem && a.redeem.output && a.internalPubkey) {
      ...
      if (!outputKey)
        return;
      const controlBock = NBuffer3.concat(
        [
          NBuffer3.from([o.redeemVersion | outputKey.parity]),
          a.internalPubkey
        ].concat(path)
      );
...
// ts_src/address.ts
import { bech32 as bech323, bech32m as bech32m2 } from "bech32";
import * as bs58check3 from "bs58check";
...
function fromBech32(address) {
  ...
  if (result) {
    ...
    result = bech32m2.decode(address);
    version = result.words[0];
    if (version === 0)
      throw new TypeError(address + " uses wrong encoding");
  }
  const data = bech323.fromWords(result.words.slice(1));
  return {
    version,
    prefix: result.prefix,
    data: Buffer.from(data)
...
function toBech32(data, version, prefix) {
  const words = bech323.toWords(data);
  words.unshift(version);
  return version === 0 ? bech323.encode(prefix, words) : bech32m2.encode(prefix, words);
}
...
import * as varuint3 from "bip174/src/lib/converter/varint";
import { checkForInput, checkForOutput } from "bip174/src/lib/utils";

// ts_src/psbt/psbtutils.ts
import * as varuint2 from "bip174/src/lib/converter/varint";
